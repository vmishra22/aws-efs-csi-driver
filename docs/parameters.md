...
| az                    |        | ""              | true     | Used for cross-account mount. If specified, the mount target in the given AZ is used. If not specified, the driver passes all available mount target IPs to each node, which selects the mount target matching its own AZ.  |
